...
export default async function BulkRefDomainsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (historyId) {
    const saved = await getBlBulkRdResults<BulkRdItem>(historyId);
    if (saved) { items = saved; activeEntry = (await getBlBulkRdHistory()).find((e) => e.id === historyId) ?? null; }
    ...
    if (!creds) { error = 'DataForSEO credentials missing. Configure them in Settings.'; }
      ...
      if (!error && items.length > 0) {
        ...
        await saveBlBulkRd(entry, items);
      }
    }
  }

  const history = await getBlBulkRdHistory();
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {history.map((entry) => {
              ...
              return (
                <Link key={entry.id} href={`/dashboard/backlinks/bulk-referring-domains?history_id=${entry.id}#results`}
