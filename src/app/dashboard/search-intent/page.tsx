...
export default async function SearchIntentPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (historyId) {
    const saved = await getSearchIntentResults<IntentItem>(historyId);
    if (saved) { items = saved; activeEntry = (await getSearchIntentHistory()).find((e) => e.id === historyId) ?? null; }
    ...
    if (!creds) { error = 'DataForSEO credentials missing. Configure them in Settings.'; }
      ...
      if (!error && items.length > 0) {
        ...
        await saveSearchIntentSearch(entry, items);
      }
    }
  }

  const history = await getSearchIntentHistory();
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {history.map((entry) => {
              ...
              return (
                <Link key={entry.id} href={`/dashboard/search-intent?history_id=${entry.id}#results`}
